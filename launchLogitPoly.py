"""
launchLogitPoly.py
Homework 4: Predicting Home Runs with Logistic Regression (Polynomial)
...
based on the launch angle and launch speed. The regression includes polynomial features
(degree 2 by default), and the script outputs the mean squared error and the model's coefficients, one per line.
...
Example command:
$ python launchLogitPoly.py all2024.csv
"""
...
def plot_decision_boundary(X, y, model, poly):
    ...
    # Transform grid values into polynomial features
    X_poly_grid = poly.transform(np.c_[xx.ravel(), yy.ravel()])

    # Predict class for every point in the grid
    Z = model.predict(X_poly_grid)
    ...
    plt.title('Decision Boundary and Data Points (Polynomial)')
    plt.show()
